...
A = load('data/FeN_I6_1.ascii');
TaI6 = A(:,5);
RI6 = A(:,3); 
RRI6 = (RI6)./(RI6(1))*100;

% data from Irrad8 RvsTa 2nd take
C = load('data/resistancevsTa2.dat'); 
Ta8 = C(:,1);
R8_2 = C(:,2);
i = [2:2:size(R8_2)];
R8_2 = R8_2(i)*1594;
...
A = load('data/FeN56_8min_incub.dat');
Ta = A(:,1); C = [C A(:,2)/A(1,2)*100];
A = load('data/FeN57_8min_incub.dat');
Ta = A(:,1); C = [C A(:,2)/A(1,2)*100]; 
A = load('data/FeN58_8min_incub.dat');
Ta = A(:,1); C = [C A(:,2)/A(1,2)*100];
